...
class Tile:
    def __init__(self, char_pos:int, speed:int, action:int, color, color_int=-1):
...
def display_tiles(TILES, playable_area, player_pos, chars, player_icons, player_direction, pos_of_chars, monsters, monsters_on_map, inventory=(("", ""),)):
    """
    ...
    for x in range(player_pos[0] - (playable_area[0] // 2), (playable_area[0] // 2) + player_pos[0]):
        for y in range(player_pos[1] - (playable_area[1] // 2), (playable_area[1] // 2) + player_pos[1]):
            ...
            # If the current coords are those of a monster
                ...
                if pos_of_chars[TILES[x][y].char_pos] == "0":
                    ...
                    colors_around = ""
                    try:
                        if pos_of_chars[TILES[x][y-1].char_pos] == "0":
                    ...
                    try:
                        if pos_of_chars[TILES[x+1][y].char_pos] == "0":
            ...
            # If the current coords aren't those of a player
                ...
                elif pos_of_chars[TILES[x][y].char_pos] == "1":
                    try:
                ...
                elif pos_of_chars[TILES[x][y].char_pos] == "2":
                    try:
                ...
                elif pos_of_chars[TILES[x][y].char_pos] == "3":
                    try:
