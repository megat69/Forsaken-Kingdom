	...
	for x in range(player_pos[0] - (playable_area[0] // 2), (playable_area[0] // 2) + player_pos[0]):
		for y in range(player_pos[1] - (playable_area[1] // 2), (playable_area[1] // 2) + player_pos[1]):
			...
			# If the current coords are those of a monster
				...
				final_str += color_around + char_around + Fore.RED + monster.monster.monster_char + color_around + Fore.RESET + char_around + Back.RESET
				del char_around
				del color_around
			# If the current coords aren't those of a player
				...
				# otherwise, we work out where to place it
					...
					try:
						final_str += TILES[x][y].color + chars[TILES[x][y].char_pos] + TILES[x][y+1].color + chars[TILES[x][y+1].char_pos] * 2 + Back.RESET
					except IndexError:
						final_str += TILES[x][y].color + chars[TILES[x][y].char_pos] + " " * 2 + Back.RESET
				elif TILES[x][y].pos_of_char == 2:
					try:
						final_str += TILES[x][y-1].color + chars[TILES[x][y-1].char_pos] + TILES[x][y].color + chars[TILES[x][y].char_pos] \
						             + TILES[x][y+1].color + chars[TILES[x][y+1].char_pos] + Back.RESET
					except IndexError:
						try:
							try:
								final_str += TILES[x+1][y].color + chars[TILES[x+1][y].char_pos] + TILES[x][y].color + chars[TILES[x][y].char_pos] \
								             + TILES[x+1][y].color + chars[TILES[x+1][y].char_pos] + Back.RESET
							except IndexError:
								final_str += TILES[x-1][y].color + chars[TILES[x-1][y].char_pos] + TILES[x][y].color + chars[TILES[x][y].char_pos] + \
								             TILES[x-1][y].color + chars[TILES[x-1][y].char_pos] + Back.RESET
						except IndexError:
							final_str += TILES[x][y].color + " " + chars[TILES[x][y].char_pos] + " " + Back.RESET
				elif TILES[x][y].pos_of_char == 3:
					try:
						final_str += TILES[x][y-1].color + chars[TILES[x][y-1].char_pos] * 2 + TILES[x][y].color + chars[TILES[x][y].char_pos] + Back.RESET
					except IndexError:
						final_str += TILES[x][y].color + " " * 2 + chars[TILES[x][y].char_pos] + Back.RESET
				...
				final_str += color_around + char_around + Style.BRIGHT + player_icons[player_direction] + Style.RESET_ALL + color_around + char_around + Back.RESET
				del char_around
				del color_around
		try:
			if inventory == (("", ""),) or inventory[counter][0] == "": raise IndexError
			final_str += "\t" + str(inventory[counter][0]) + " : " + str(inventory[counter][1] if not isinstance(inventory[counter][1], (list, tuple)) else ", ".join(element for element in inventory[counter][1])) + "\n"
		except IndexError:
			final_str += "\n"
		finally:
			counter += 1
